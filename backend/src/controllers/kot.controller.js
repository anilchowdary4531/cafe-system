...
import { dispatchKotPrint, reprintKot, updateKotStatus } from "../services/kotService.js";

/**
 * Fetch KOTs for a restaurant with status, station & table filters
 */
export const getKots = async (req, res) => {
  try {
    ...
    const { status, stationId, tableNo, tableSessionId, limit = 50 } = req.query;
...
export const updateStatus = async (req, res) => {
  try {
    ...
    const updated = await updateKotStatus({
      prisma: req.prisma,
      kotId,
      restaurantId,
      nextStatus: status,
