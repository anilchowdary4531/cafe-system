...
export default function GoogleSignInButton({ onSuccessRedirect, className = "" }) {
  ...
  const handleGoogleSuccess = async (googleUserPayload) => {
    try {
      ...
      if (res.data?.requiresInfo) {
        setPendingGoogleData({
          googleId: res.data.googleId || googleUserPayload.googleId,
          email: res.data.email || googleUserPayload.email,
          name: res.data.name || googleUserPayload.name || "",
          picture: res.data.picture || googleUserPayload.picture,
          credential: googleUserPayload.credential,
        });
        setModalOpen(true);
        return;
      }
  ...
  return (
    <>
      ...
      <GoogleCompleteProfileModal
        isOpen={modalOpen}
        initialData={pendingGoogleData || {}}
        loading={loading}
        onSubmit={(fullPayload) => {
          handleGoogleSuccess(fullPayload);
        }}
